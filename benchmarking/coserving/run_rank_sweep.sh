...
RANK_VALS=(4 8 32 64)   # rank=16 already done
...
OUTPUT_FOLDER="../../output/e2e/rank_sweep"
...
run_experiment() {
    ...
    lora_rank=${RANK_VALS[$rank_index]}

    TRACE_FILE="${TRACES_FOLDER_}/${trace}_${MAX_SEQ_LEN}_${qps}_qps.json"
    OUTPUT_FILE="${OUTPUT_FOLDER}/output/${MODEL_NAME//\//_}_${trace}_bz_${BATCH_SIZE}_tokens_per_batch_${MAX_TOKENS_PER_BATCH}_kv_cache_slots_${NUM_KV_CACHE_SLOTS}_${NUM_BWD_LAYERS}_bwd_layers_${qps}_qps_${lora_rank}_${PEFT_SUPPORT_MODE}.json"
    LOG_FILE="${OUTPUT_FOLDER}/logs/${MODEL_NAME//\//_}_${trace}_bz_${BATCH_SIZE}_tokens_per_batch_${MAX_TOKENS_PER_BATCH}_kv_cache_slots_${NUM_KV_CACHE_SLOTS}_${NUM_BWD_LAYERS}_bwd_layers_${qps}_qps_${lora_rank}_${PEFT_SUPPORT_MODE}.log"
...
echo "Total experiments: $((${#MODEL_NAMES[@]} * ${#QPS_vals[@]} * ${#RANK_VALS[@]}))"
echo ""
...
for model_index in $(seq 0 $((${#MODEL_NAMES[@]} - 1))); do
    ...
    for rank_index in $(seq 0 $((${#RANK_VALS[@]} - 1))); do
        for qps_index in $(seq 0 $((${#QPS_vals[@]} - 1))); do
            run_experiment $model_index $qps_index $rank_index
        done
    done
done
